...
from mario_brain.env_factory import create_parallel_training_env, create_training_env
from mario_brain.location import LATEST_MODEL_PATH, LOG_DIR, MODEL_DIR, MODEL_PATH
from mario_brain.schedules.linear_schedule import CappedLinearSchedule


def callback_list() -> CallbackList:
    checkpoint_callback = CheckpointCallback(
        save_freq=1_000_000, save_path=MODEL_DIR, verbose=1
    )
...
def init(total_timesteps, parallel=8):
    ...
    if parallel > 1:
        env = create_parallel_training_env(
            version="v0",
            parallel=parallel,
...
def get_model(env, device, model_path):
    ...
    if new_model:
        ...
        model = PPO(
            ...
            learning_rate=CappedLinearSchedule(0.00001, 0.000001),
            ent_coef=0.02,
            batch_size=2048,
            n_steps=1024,
            device=device,
        ...
        model = PPO.load(
            ...
            env=env,
            device=device,
            custom_objects=dict(learning_rate=CappedLinearSchedule(0.00001, 0.000001)),
        )
    return model, new_model


def train(
    parallel: int = 8, total_timesteps: int = 50_000_000, model_path: str = MODEL_PATH
):
    env, device = init(total_timesteps, parallel)

    model, reset_num_timesteps = get_model(env, device, model_path)
    print(
        f"Number of timesteps trained: {model.num_timesteps}, reset_num_timesteps: {reset_num_timesteps}"
    )
    model.learn(
        total_timesteps=total_timesteps,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--parallel", type=int, default=8, help="Number of parallel environments"
    )
    parser.add_argument(
        "--timesteps", type=int, default=10_000_000, help="Total timesteps to train"
    )
    parser.add_argument(
        "--model",
        type=str,
        default=LATEST_MODEL_PATH,
