...
def create_env(version="v0", render_mode=None, reward_summary=False):
    ...
    env = gym_super_mario_bros.make(
        "SuperMarioBros-" + version, render_mode=render_mode
    )
...
def create_parallel_training_env(
    version="v0", render_mode=None, parallel=4, record_video=False, total_timesteps=1000
):
    env = SubprocVecEnv(
        [
            lambda: create_env(version=version, render_mode=render_mode)
